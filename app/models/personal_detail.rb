class PersonalDetail < ActiveRecord::Base
  ...
  # Constants for validation, ideally these would be an enumeration or in a config file or stored in the
  # database to send to the broker.
  ...
  validates_presence_of(:forenames, :surname, :date_of_birth, :email, :title, :license_type, :license_period,
                        :telephone_number, :occupation, :user_id)
